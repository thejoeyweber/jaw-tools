...
Quickly create empty test files for new features, co-located in the feature folder, following conventional structures. This helps ensure that tests are created as part of the development process.

**Usage:**
```bash
npx jaw-tools test-scaffold <feature-name-slug> [options]
# or
npx jaw-tools ts <feature-name-slug> [options]
```

**Arguments & Options:**

*   `<feature-name-slug>`: The slug for the feature (e.g., `user-profile` or `new-api-endpoint`). This slug is used to create a feature-specific directory under `features/` (e.g., `features/user-profile/`).
*   `--types type1,type2,...`: A comma-separated list of specific test suite types to generate (e.g., `unit,integration`). This overrides the default types defined in your configuration.
*   `--all`: Generate test stubs for all default test types specified in the `testScaffold.defaultTypes` configuration. This is useful if you've customized `defaultTypes` and want to ensure all are scaffolded for a feature.
*   `--dry-run`: Show what files would be created or overwritten without actually writing them to disk. This is helpful for previewing changes.
*   `--force`: Overwrite existing test stub files if they already exist. By default, the command will skip existing files.

**File Structure & Templates:**

*   Test files are typically generated within a `__tests__` subdirectory inside the feature folder: `features/<feature-name-slug>/__tests__/`.
*   The command uses templates from a configurable directory (default: `templates/tests/` relative to your project root). Template files are expected to follow a naming convention like `unit.test.template.ts`, `api.test.template.ts`, etc.
*   The generated file names also follow a configurable pattern (default: `{feature}.{type}.test.ts`).

**Placeholders in Templates:**

When your test templates (e.g., `templates/tests/unit.test.template.ts`) are processed, the following placeholders are replaced:
*   `<FEATURE_NAME>`: Replaced with the `<feature-name-slug>`.
*   `<IMPORT_PATH>`: Replaced with a relative path (e.g., `../../<feature-name-slug>`) to allow the test file to import from the main feature file.
*   `// INSERT_TODO_MARKER_HERE`: This specific comment in your template will be replaced by the `todoMarker` string defined in your configuration (which itself can contain `<FEATURE_NAME>`). If this comment is not found, the `todoMarker` is appended to the end of the generated file.
...
module.exports = {
  ...
  testScaffold: {
    templateDir: 'templates/tests/', // Default directory for test stub templates
    fileNaming: '{feature}.{type}.test.ts', // Naming pattern for generated files ({feature}, {type})
    defaultTypes: ['unit', 'integration', 'a11y', 'api'], // Test types generated by default or with --all
    todoMarker: '// TODO: write meaningful assertions for <FEATURE_NAME>' // Placeholder text inserted into stubs
  },
...
| `jaw-tools test-scaffold <feature-slug> [--types type1,...] [--all] [--dry-run] [--force]` <br> `jaw-tools ts <feature-slug> ...` | Generate test file stubs for a new feature. |
