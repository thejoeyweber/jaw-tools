...
const fs = require('fs-extra'); // Directly require fs-extra
const path = require('path');
const { 
  // ensureDir removed from here, will use fs.ensureDirSync from fs-extra
  resolveFileConflict, 
  getRenameSuffix, 
  createInterface,
  formatError // Import formatError
} = require('../src/utils');
 ...
 * Assumes it's called with `(config, argv)` from `executeCommandModule`.
 * The `force` option is expected to be in `argv.force`.
 * 
 * @async
 * @param {Object} config - The jaw-tools configuration object.
 * @param {Object} argv - The yargs argv object, expected to contain a `force` boolean property.
 * @returns {Promise<Object>} Result object indicating success or failure, along with scaffolding stats.
 *                            Example: `{ success: true, filesProcessed: N, ... }` or
 *                                     `{ success: false, error: 'message', ... }`
 */
async function scaffold(config, argv) {
  const force = argv.force || false; // Get force from argv, default to false
  const packageDir = path.dirname(path.dirname(__filename)); // jaw-tools package root
  const projectRoot = config.__projectRoot || process.cwd(); // User's project root
  
  const scaffoldSourceRoot = path.join(packageDir, 'templates', 'scaffold_root');
  
  // Use the configured docs directory instead of scaffoldTargetRootDir
  ...
  if (!fs.existsSync(scaffoldSourceRoot)) {
    // console.error calls removed as the error message will be thrown
    throw new Error(`Scaffold source directory not found: ${scaffoldSourceRoot}. This directory should contain template files. Run "npx jaw-tools doctor" to diagnose.`);
  }
  ...
  try {
    ...
    if (result.aborted) {
      if (rl) rl.close();
      throw new Error('Scaffolding aborted by user.');
    }
    
    // Copy user guide if it exists
    if (fs.existsSync(userGuideSource)) {
      // Ensure docs directory exists
      fs.ensureDirSync(userGuideDestDir); // Replaced utils.ensureDir with fs-extra.ensureDirSync
      ...
      if (fs.existsSync(userGuideDestFile)) {
        if (force) {
          ...
          if (action === 'skip') {
            ...
            if (rl) rl.close();
            throw new Error('User guide copy aborted by user.');
          }
        }
      } else {
        fs.copyFileSync(userGuideSource, userGuideDestFile);
    ...
    if (rl) rl.close();
    // On success, we can return the result object if it's useful, or just complete.
    // executeCommandModule currently expects functions that throw on error OR return {success: boolean}
    // Since we are moving to throw on error, this explicit return of success is not strictly needed
    // by executeCommandModule anymore, but can be kept if the stats are useful.
    return { 
      // success: true, // Not strictly needed if errors are thrown for failure
    ...
    // console.error call removed, error will be propagated by throw
    if (rl) rl.close();
    if (err.isFormatted) throw err; // Re-throw if already formatted
    throw formatError('Error during scaffolding', { originalError: err });
  }
}
