 ...
 */
async function refreshTemplates(config, options = {}) {
  if (!config) {
    console.error("❌ Configuration object is required for refreshTemplates.");
 ...
 * @returns {Promise<object>} Result object
 */
async function refreshRepomixProfiles(config) {
  if (!config) {
    console.error("❌ Configuration object is required for refreshRepomixProfiles.");
    return { success: false, error: "Configuration object is required." };
  }
  try {
