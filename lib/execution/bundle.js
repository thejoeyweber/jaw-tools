...
function parsePrdInfo(prdFilePath) {
  ...
  if (!frontMatter.prdId) {
    throw new Error('PRD file must have prdId in front-matter');
  }
  
  // Ensure name is defined and a string
  if (!frontMatter.name) {
    throw new Error('PRD file must have name in front-matter');
  }
  
  // Convert name to string if it's not already
  const nameStr = typeof frontMatter.name === 'string' 
    ? frontMatter.name 
  ...
  return {
    ...
    sanitizedName: nameStr.replace(/[\/\\:*?"<>|]/g, '-').replace(/\s+/g, '-').trim(),
    content: prdContent,
    frontMatter
  };
}
...
function compileTemplate(template, context) {
  return template.replace(/\{\{([^}]+)\}\}/g, (match, key) => {
    ...
    if (trimmedKey.startsWith('#if ')) {
      ...
      return conditionVar ? '' : '<!-- Condition not met -->';
    }
    if (trimmedKey === '/if') {
      return '';
    }
