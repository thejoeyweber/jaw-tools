...
/**
 * Parses command line arguments for path, ignore patterns, and fix mode.
 * @param {string[]} argsArray - Array of command line arguments.
 * @returns {{searchPath: string, ignorePatterns: string[], fixMode: boolean}}
 */
function parseDocLintArgs(argsArray) {
  ...
  return { searchPath, ignorePatterns, fixMode };
}
...
function runDocLint(cliArgs, config) {
  // console.log('Executing runDocLint with CLI args:', cliArgs); // Keep for debugging if necessary

  const { searchPath, ignorePatterns, fixMode } = parseDocLintArgs(cliArgs);
  if (fixMode) {
  ...
  try {
    ...
    detailedMessages.forEach(report => {
      let statusIcon = '✔';
      if (report.status === 'failed') statusIcon = '✖';
      else if (report.status === 'fixed') statusIcon = '🛠️';
      
      console.log(`${statusIcon} ${report.filepath}`);
