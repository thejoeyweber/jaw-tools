...
class MiniPrdManager {
  constructor(projectRoot) {
    ...
    this.docsDir = path.join(projectRoot, '_docs/mini-prds');
    this.templatePath = path.join(projectRoot, '_docs/templates/mini-prd-template.md');
  ...
  getAllPrds() {
    ...
    return files.map(file => {
      try {
        ...
        console.warn(`Warning: Could not parse PRD config file: ${file}`);
        return null;
      }
    }).filter(Boolean);
  }
   ...
   */
  savePrd(prd) {
    const configPath = path.join(this.configDir, `${prd.id}.json`);
    fs.writeJsonSync(configPath, prd, { spaces: 2 });
    return configPath;
  }
  ...
  syncFromMarkdown() {
    ...
    for (const file of files) {
      try {
        ...
        if (fs.existsSync(configPath)) {
          // Update existing PRD
          const prd = JSON.parse(fs.readFileSync(configPath, 'utf8'));
          ...
          // Update from front-matter
          prd.name = parsed.data.name;
          prd.description = parsed.data.description;
          prd.includes = parsed.data.includes;
          prd.excludes = parsed.data.excludes;
          prd.plannedFiles = parsed.data.plannedFiles;
          prd.updatedAt = new Date().toISOString();
