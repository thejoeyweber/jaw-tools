 ...
 */
function compile(promptFilePath, options = {}, config) {
  if (!config) {
...
if (require.main === module) {
  ...
  if (!templatePathRelative) {
    console.error('Usage: node lib/compile-prompt.js <path-to-template> [options]');
    // Try to load config for example path
    let examplePath = '_docs/prompts/example.md'; // Default example
    try {
      const configManager = require('../src/config-manager');
      const tempConfig = configManager.getConfig(); // Use a temporary config for example path
      if (tempConfig && tempConfig.directories && tempConfig.directories.prompts) {
        examplePath = path.join(tempConfig.directories.prompts, 'example.md');
      }
    } catch (err) {
      // Use default example path if config loading fails
    }
  ...
  try {
    ...
    console.error(err.message);
    console.error('Please ensure jaw-tools.config.js is present and correctly formatted.');
    process.exit(1);
  }
  ...
  try {
    const result = compile(templatePathRelative, options, config);
    process.exit(result.success ? 0 : 1);
  } catch (err) {
    console.error(`Error during prompt compilation: ${err.message}`);
    process.exit(1);
  }
}
