...
const [,, templatePathRelative] = process.argv;
if (!templatePathRelative) {
  console.error('Usage: jaw-tools compile-prompt <path-to-template>');
  console.error(`Example: jaw-tools compile-prompt ${config.directories.prompts}/example.md`);
  process.exit(1);
}

const templatePath = path.resolve(templatePathRelative);
...
for (const pathPattern of pathsToProcess) {
  ...
  if (pathPattern.includes('*')) {
    ...
    // Regular file path
    try {
      const absoluteFilePath = path.resolve(pathPattern);
      const fileContent = fs.readFileSync(absoluteFilePath, 'utf8');
      replacements[pathPattern] = fileContent;
    } catch (err) {
      replacements[pathPattern] = `<!-- ERROR: Could not read file ${pathPattern} -->`;
    }
  }
}
